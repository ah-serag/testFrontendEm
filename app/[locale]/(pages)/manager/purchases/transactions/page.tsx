...
import { useGetSupplierTransactionsQuery } from "@/redux/features/supplierApiSlice";
...
import { Loader2, Search, ArrowDownLeft, ArrowUpRight, CalendarDays, Receipt, Hash, User, AlignLeft, ChevronRight, ChevronLeft, Filter, RotateCcw, Scale } from "lucide-react";
...
export default function SupplierTransactionsPage() {
  ...
  // ===== Fetch Data =====
  const { data: response, isLoading, isFetching, refetch } = useGetSupplierTransactionsQuery({
    page,
    limit: 15,
  ...
  const getTransactionBadge = (type: string) => {
    switch (type) {
      case "INVOICE":
        return (
          <Badge className="bg-sky-50 text-sky-800 border w-28 rounded-3xl shadow-sm border-sky-200 px-2.5 py-1 text-[11px] font-bold flex items-center justify-center gap-1">
            <ArrowDownLeft size={14} /> فاتورة شراء
          </Badge>
        );
      case "PAYMENT":
        return (
          <Badge className="bg-emerald-50 text-emerald-800 border w-28 rounded-3xl shadow-sm border-emerald-200 px-2.5 py-1 text-[11px] font-bold flex items-center justify-center gap-1">
            <ArrowUpRight size={14} /> سند سداد
          </Badge>
        );
      case "RETURN":
        return (
          <Badge className="bg-rose-50 text-rose-800 border w-28 rounded-3xl shadow-sm border-rose-200 px-2.5 py-1 text-[11px] font-bold flex items-center justify-center gap-1">
            <RotateCcw size={14} /> مرتجع
          </Badge>
        );
      case "OPENING_BALANCE":
        return (
          <Badge className="bg-slate-50 text-slate-700 border w-28 rounded-3xl shadow-sm border-slate-200 px-2.5 py-1 text-[11px] font-bold flex items-center justify-center gap-1">
            <Scale size={14} /> رصيد افتتاحي
          </Badge>
        );
      default:
        return (
          <Badge className="bg-slate-50 text-slate-600 border w-28 rounded-3xl shadow-sm border-slate-200 px-2.5 py-1 text-[11px] font-bold flex items-center justify-center gap-1">
            {type}
          </Badge>
        );
    }
  ...
  return (
    <div className="p-4 md:p-6 max-w-dvw mx-auto space-y-5" dir="rtl">
      ...
      <div className="flex flex-row justify-between flex-wrap items-center gap-4 bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200/60">
       <div className="flex gap-3">
        ...
        <div>
          <h1 className="text-lg md:text-xl font-normal text-slate-800">حركات الموردين</h1>
          <p className="text-[12px] md:text-[13px] text-slate-500 font-medium">متابعة الفواتير والمسددات الخاصة بالموردين</p>
        </div>
       </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200/60 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="relative w-full md:w-[400px]">
            ...
            <Input 
              placeholder="ابحث باسم المورد أو رقم الحركة..."
              value={searchTerm}
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <Select value={typeFilter} onValueChange={(val) => { setTypeFilter(val); setPage(1); }}>
              ...
              <SelectContent className="rounded-xl">
                ...
                <SelectItem value="INVOICE">فاتورة شراء</SelectItem>
                <SelectItem value="PAYMENT">سند سداد</SelectItem>
      ...
      {isLoading || isFetching ? (
        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl border border-slate-200/60 shadow-sm gap-3">
          ...
          <p className="text-[12px] text-slate-500 font-medium">جاري جلب حركات الموردين...</p>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-right text-[12px]">
                <thead className="bg-slate-50 border-b border-slate-100 text-slate-500 font-bold uppercase tracking-wider text-[11px]">
                  <tr>
                    <th className="px-5 py-3 whitespace-nowrap">رقم الحركة</th>
                    <th className="px-5 py-3 whitespace-nowrap">التاريخ</th>
                    <th className="px-5 py-3 whitespace-nowrap">المورد / الشركة</th>
                    <th className="px-5 py-3 whitespace-nowrap text-center">النوع</th>
                    <th className="px-5 py-3 whitespace-nowrap text-left">المبلغ</th>
                    <th className="px-5 py-3 whitespace-nowrap text-left">الرصيد بعدها</th>
                    <th className="px-5 py-3 whitespace-nowrap">المرجع</th>
                    <th className="px-5 py-3 whitespace-nowrap w-48">البيان / ملاحظات</th>
                  </tr>
                </thead>
                    ...
                    <tr key={tr.id} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      <td className="px-5 py-3 font-bold text-slate-700 whitespace-nowrap">
                        <div className="flex flex-col">
                          <span>{tr.supplier_name}</span>
                          {tr.company_name && <span className="text-[10px] text-slate-400 font-normal">{tr.company_name}</span>}
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-3 font-mono font-bold text-[14px] text-left whitespace-nowrap">
                        <span className={tr.transaction_type === "PAYMENT" || tr.transaction_type === "RETURN" ? "text-emerald-600" : "text-rose-600"}>
                          {Number(tr.amount).toLocaleString()} ج
                        </span>
                      </td>
                      <td className="px-5 py-3 font-mono font-bold text-[13px] text-left whitespace-nowrap text-slate-700">
                        {Number(tr.balance_after).toLocaleString()} ج
                      </td>
              ...
              <div key={tr.id} className="bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm space-y-3 relative overflow-hidden">
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    ...
                    <h3 className="font-bold text-[13px] text-slate-800">{tr.supplier_name}</h3>
                    {tr.company_name && (
                      <p className="text-[11px] text-slate-500">{tr.company_name}</p>
                    )}
                  </div>
                ...
                <div className="bg-slate-50 rounded-xl p-3 flex justify-between items-center border border-slate-100">
                  <div className="space-y-0.5">
                    <span className="text-[10px] font-bold text-slate-400">المبلغ</span>
                    <p className={`font-mono font-bold text-[15px] ${tr.transaction_type === "PAYMENT" || tr.transaction_type === "RETURN" ? "text-emerald-600" : "text-rose-600"}`}>
                      {Number(tr.amount).toLocaleString()} ج
                    </p>
                  </div>
                  <div className="text-left space-y-0.5">
                    <span className="text-[10px] font-bold text-slate-400">الرصيد بعدها</span>
                    <p className="font-bold text-[14px] text-slate-700 font-mono">
                      {Number(tr.balance_after).toLocaleString()} ج
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  ...
                  <div className="flex items-center gap-1.5 text-slate-500 bg-slate-50 p-1.5 rounded-lg border border-slate-100">
                    ...
                    <span className="truncate">{tr.created_by_name || "النظام"}</span>
                  </div>
                </div>
                ...
                {tr.reference_type && (
                  <div className="text-[10px] text-slate-400 font-mono text-left pt-1 border-t border-slate-100 mt-2">
